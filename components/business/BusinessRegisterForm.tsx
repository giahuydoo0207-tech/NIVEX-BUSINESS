...
export function BusinessRegisterForm() {
  ...
  function validateCurrentStep() {
    ...
    if (
      step === 2 &&
      (!form.legalName.trim() || !form.registrationNumber.trim())
    ) {
    ...
    if (step === 3) {
      if (!form.representativeTitle.trim())
        return "Nhập chức vụ của người đại diện.";
      if (!form.confirmedAuthority)
        return "Xác nhận bạn có quyền đại diện cho tổ chức.";
  ...
  async function submitRegistration() {
    ...
    const {
      ...
      confirmedAuthority: _authority,
      ...registration
    } = form;
    try {
      localStorage.setItem(
        "nivex.demo.organization",
        JSON.stringify(registration),
      );
    } catch {
      setSubmitting(false);
      return setError(
        "Không lưu được hồ sơ. Kiểm tra quyền lưu trữ của trình duyệt.",
      );
    }
    ...
    <div className="business-register-grid">
      <section className="register-context">
        ...
        <div className="register-demo-note">
          Hồ sơ được tạo trong chế độ thử nghiệm Devnet. Xác minh KYB thật sẽ
          được nối qua đối tác ở giai đoạn production.
        </div>
      </section>
          ...
          <div className="form-grid">
            ...
            <label className="field">
              ...
              <select
                value={form.type}
                onChange={(event) =>
                  update("type", event.target.value as OrganizationType)
                }
              >
            ...
            <label className="field">
              <span>Mã doanh nghiệp / mã số thuế</span>
              <input
        ...
        {step === 3 && (
          <div className="form-grid">
            <label className="field full">
              ...
              <input value={form.ownerFullName} readOnly />
            </label>
            <label className="field full">
              <span>Chức vụ</span>
              <input
                ...
                onChange={(event) =>
                  update("representativeTitle", event.target.value)
                }
                placeholder="Ví dụ: Giám đốc, Finance Manager"
              />
            </label>
            <label className="check-field full">
              <input
                ...
                onChange={(event) =>
                  update("confirmedAuthority", event.target.checked)
                }
              />
              <span>
                Tôi xác nhận có quyền tạo và quản lý không gian thanh toán của
                tổ chức này.
              </span>
            </label>
            <label className="check-field full">
